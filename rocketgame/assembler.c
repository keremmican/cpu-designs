...
//========================================   FIRST PASS  ======================================================
            while (token)
            {
                ...
                {
                    op1 = strtok(NULL,"\n\t\r ");                           //read the label string
                    jumptable[noofjumps].location = counter;                //write the jump instruction's location into the jumptable
                    op2=(char*)malloc(sizeof(op1));                         //allocate space for the label
                    strcpy(op2,op1);                                        //copy the label into the allocated space
                    jumptable[noofjumps].name=op2;                          //point to the label from the jumptable
                    noofjumps++;                                            //skip to the next empty location in jumptable
                ...
                {
                    op1 = strtok(NULL,"\n\t\r ");                           //get the 1st operand of add, which is the destination register
                    op2 = strtok(NULL,"\n\t\r ");                           //get the 2nd operand of add, which is the source 1 register
                    op3 = strtok(NULL,"\n\t\r ");                           //get the 3rd operand of add, which is the source 2 register
                    chch = (op1[0]-48)| ((op2[0]-48)<<6)|((op3[0]-48)<<3);  //form bits 11-0 of machine code. 48 is ASCII value of '0'
                    program[counter]=0x7000+((chch)&0x01ff);                //form the instruction and write it to memory
                    counter++;                                              //skip to the next empty location in memory
                }
                else if (strcmp(token,"sub")==0) //----------------- SUB -------------------------------
                {
                    op1 = strtok(NULL,"\n\t\r ");                           //get the 1st operand of sub, which is the destination register
                    op2 = strtok(NULL,"\n\t\r ");                           //get the 2nd operand of sub, which is the source 1 register
                    op3 = strtok(NULL,"\n\t\r ");                           //get the 3rd operand of sub, which is the source 2 register
                    chch = (op1[0]-48)| ((op2[0]-48)<<6)|((op3[0]-48)<<3);  //form bits 11-0 of machine code. 48 is ASCII value of '0'
                    program[counter]=0x7200+((chch)&0x01ff);                //form the instruction and write it to memory
                    counter++;                                              //skip to the next empty location in memory
                }
                else if (strcmp(token,"and")==0) //----------------- AND -------------------------------
                {
                    op1 = strtok(NULL,"\n\t\r ");                           //get the 1st operand of and, which is the destination register
                    op2 = strtok(NULL,"\n\t\r ");                           //get the 2nd operand of and, which is the source 1 register
                    op3 = strtok(NULL,"\n\t\r ");                           //get the 3rd operand of and, which is the source 2 register
                    chch = (op1[0]-48)| ((op2[0]-48)<<6)|((op3[0]-48)<<3);  //form bits 11-0 of machine code. 48 is ASCII value of '0'
                    program[counter]=0x7400+((chch)&0x01ff);                //form the instruction and write it to memory
                    counter++;                                              //skip to the next empty location in memory
                }
                else if (strcmp(token,"or")==0) //----------------- OR -------------------------------
                {
                    op1 = strtok(NULL,"\n\t\r ");                           //get the 1st operand of or, which is the destination register
                    op2 = strtok(NULL,"\n\t\r ");                           //get the 2nd operand of or, which is the source 1 register
                    op3 = strtok(NULL,"\n\t\r ");                           //get the 3rd operand of or, which is the source 2 register
                    chch = (op1[0]-48)| ((op2[0]-48)<<6)|((op3[0]-48)<<2);  //form bits 11-0 of machine code. 48 is ASCII value of '0'
                    program[counter]=0x7600+((chch)&0x01ff);                //form the instruction and write it to memory
                    counter++;                                              //skip to the next empty location in memory
                }
                else if (strcmp(token,"xor")==0) //----------------- XOR -------------------------------
                {
                    op1 = strtok(NULL,"\n\t\r ");                           //get the 1st operand of xor, which is the destination register
                    op2 = strtok(NULL,"\n\t\r ");                           //get the 2nd operand of xor, which is the source 1 register
                    op3 = strtok(NULL,"\n\t\r ");                           //get the 3rd operand of xor, which is the source 2 register
                    chch = (op1[0]-48)| ((op2[0]-48)<<6)|((op3[0]-48)<<3);  //form bits 11-0 of machine code. 48 is ASCII value of '0'
                    program[counter]=0x7800+((chch)&0x01ff);                //form the instruction and write it to memory
                    counter++;                                              //skip to the next empty location in memory
                }
                else if (strcmp(token,"not")==0) //----------------- NOT -------------------------------
                {
                    op1 = strtok(NULL,"\n\t\r ");                           //get the 1st operand of not, which is the destination register
                    op2 = strtok(NULL,"\n\t\r ");                           //get the 2nd operand of not, which is the source register
                    chch = (op1[0]-48)| ((op2[0]-48)<<3);                   //form bits 11-0 of machine code. 48 is ASCII value of '0'
                    program[counter]=0x7E00+((chch)&0x00ff);                //form the instruction and write it to memory
                    counter++;                                              //skip to the next empty location in memory
                }
                else if (strcmp(token,"mov")==0) //----------------- MOV -------------------------------
                {
                    op1 = strtok(NULL,"\n\t\r ");                           //get the 1st operand of mov, which is the destination register
                    op2 = strtok(NULL,"\n\t\r ");                           //get the 2nd operand of mov, which is the source register
                    chch = (op1[0]-48)| ((op2[0]-48)<<3);                   //form bits 11-0 of machine code. 48 is ASCII value of '0'
                    program[counter]=0x7E40+((chch)&0x00ff);                //form the instruction and write it to memory
                    counter++;                                              //skip to the next empty location in memory
                }
                else if (strcmp(token,"inc")==0) //----------------- INC -------------------------------
                {
                    op1 = strtok(NULL,"\n\t\r ");                           //get the 1st operand of inc, which is both source and destination register
                    chch = (op1[0]-48)| ((op1[0]-48)<<3);                   //form bits 11-0 of machine code. 48 is ASCII value of '0'
                    program[counter]=0x7E80+((chch)&0x00ff);                //form the instruction and write it to memory
                    counter++;                                              //skip to the next empty location in memory
                }
                else if (strcmp(token,"dec")==0) //----------------- DEC -------------------------------
                {
                    op1 = strtok(NULL,"\n\t\r ");                           //get the 1st operand of dec, which is both source and destination register
                    chch = (op1[0]-48)| ((op1[0]-48)<<3);                   //form bits 11-0 of machine code. 48 is ASCII value of '0'
                    program[counter]=0x7EC0+((chch)&0x00ff);                //form the instruction and write it to memory
                    counter++;                                              //skip to the next empty location in memory
                }
                else if (strcmp(token,"push")==0) //----------------- PUSH ------------------------------
                {
                    op1 = strtok(NULL,"\n\t\r ");                           //get the 1st operand of add, which is the source register
                    chch = ((op1[0]-48)<<6);                                //form bits 11-0 of machine code. 48 is ASCII value of '0'
                    program[counter]=0x8000+((chch)&0x01c0);                //form the instruction and write it to memory
                    counter++;                                              //skip to the next empty location in memory
                }
                else if (strcmp(token,"pop")==0) //----------------- POP -------------------------------
                {
                    op1 = strtok(NULL,"\n\t\r ");                           //get the 1st operand of add, which is the destination register
                    chch = ((op1[0]-48));                                   //form bits 11-0 of machine code. 48 is ASCII value of '0'
                    program[counter]=0x9000+((chch)&0x0007);                //form the instruction and write it to memory
                    counter++;                                              //skip to the next empty location in memory    
                }
                else if (strcmp(token,"call")==0) //----------------- CALL ------------------------------
                { 
                    op1 = strtok(NULL,"\n\t\r ");                           //get the 1st operand of call, which is the target register
                    jumptable[noofjumps].location = counter;                //write the call instruction's location into the jumptable
                    op2=(char*)malloc(sizeof(op1));                         //allocate space for the label
                    strcpy(op2,op1);                                        //copy the label into the allocated space
                    jumptable[noofjumps].name=op2;                          //point to the label from the jumptable
                    noofjumps++;                                            //skip to the next empty location in jumptable
                ...
                {
                    program[counter] = 0xB000;                              //form the instruction and write it to memory
                    counter++;                                              //skip to the next empty location in memory
